...
# Add User to be created with password and group
User = "demouser1"
Password = "demo@2023"
Groups = "isam-tenants"

# PORD & DR SERVERS with LMI user with Account management privilages
servers = {
...
def create_isam_user(server_data):
    logger.info("=== Create Request initiated ===")
    url = f"https://{server_data['ip']}/sysaccount/users/v1"
    logger.info(url)
    payload = '{"id":"' + User + '","password":"' + Password + '","groups": [{"id":"' + Groups + '"}]}'
    logger.info(payload)
    ...
    logger.info("=== Create Request finished ===")

# Send POST requests and print responses
for server, data in servers.items():
    response_text = create_isam_user(data)
